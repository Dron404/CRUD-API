...
describe('Test App scenario 3', () => {
  ...
  test('PUT /api/users/{userId} should return 200 and updated record if request is valid', async () => {
    const createResponse = await fetch('http://localhost:3005/api/users', {
      method: 'POST',
    ...
    const updateResponse = await fetch(`http://localhost:3005/api/users/${userId}`, {
      method: 'PUT',
  ...
  test('PUT /api/users/{userId} should return 400 if userId is invalid', async () => {
    const response = await fetch('http://localhost:3005/api/users/invalidUserId', {
      method: 'PUT',
      body: JSON.stringify(updatedUser),
      headers: { 'Content-Type': 'application/json' },
    });
  ...
  test('PUT /api/users/{userId} should return 404 if record does not exist', async () => {
    const response = await fetch('http://localhost:3005/api/users/c1a9d7b3-d749-4b98-b570-348a1652d088', {
      method: 'PUT',
      body: JSON.stringify(updatedUser),
      headers: { 'Content-Type': 'application/json' },
    });
